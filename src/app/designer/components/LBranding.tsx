...
const LBranding: FC<LBrandingProps> = ({}) => {
  ...
  useEffect(() => {
    const container = document.querySelector("#lPanelContainer")!;
    const element = document.querySelector("#lPanelElement")!;    
    const img = document.querySelector("#lPanelImg")!;
  ...
  }, [brandings[2].imageURL]);

  return (
    <div
      id="lPanelContainer"
      className={cn(
        brandings[2].showBrandingPicker
      ...
      style={{
        ...
        left: "1%",
        right: "45%",
        borderRadius: "300px 190px 0px 0px",
        rotate: '-6deg'
      }}
      draggable={false}
    >
      {brandings[2].imageURL ? (
        <div
          id="lPanelElement"
          className={cn(
            brandings[2].imageURL ? "block" : "hidden",
            brandings[2].showBrandingPicker
          ...
          style={{
            ...
            top: lLogo.top,
            left: lLogo.left,
            width: lLogo.width + "px",
            height: lLogo.height + "px",
          }}
        >
          <img
            id="lPanelImg"
            src={brandings[2].imageURL ? brandings[2].imageURL : ""}
            alt=""
  ...
  function makeDraggable(element: any, container: any) {
    ...
    function mouseUp() {
      ...
      if (lLogo.top < parseInt(element.style.height) * 0.8 * -1) {
        console.log("out of bounds top or left");
        element.style.top = "0px";
        element.style.left = "0px";

        setLLogo((lLogo) => ({ ...lLogo, top: 0, left: 0 }));
      } else {
  ...
  function makeResizable(element: any, container: any, img: any, capcanvasinterior: any, capcanvasouter: any) {
    const resizeHandle = document.querySelector("#leftResizer")!;
    ...
    function resizeMouseDown(e: any) {
      e.stopPropagation();
      isResizing = true;
      startWidth = lLogo.width;
      startHeight = lLogo.height;
    ...
    function resizeMouseMove(e: any) {
      if (isResizing) {
        ...
        setLLogo((lLogo) => ({ ...lLogo, width: newWidth, height: newHeight }));
      }
  ...
  function makeRotatable(element: any) {
    const rotateHandle = document.querySelector("#leftRotater")!;
